...
contract Jacpot is Ownable {
    ...
    function fundJackpot(address _addr) public payable {
        ...
        j.invested[_add] += msg.value;

        // update winners
        uint index = 0;
        while(index < j.winRates.length && j.invested[_add] <= j.invested[j.winners[index]]) {
            index++;
        }
        if(index < j.winRates.length) {
            ...
            j.winners[index] = _add;
        }

        // log
        emit JackpotFunded(_add, msg.value, block.timestamp);
    }
